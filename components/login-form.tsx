...
export function LoginForm({ onLogin, onCancel, onSwitchToSignup }: LoginFormProps) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Only proceed to onLogin if no error was thrown
      if (auth.currentUser) {
        onLogin()
      }
    } catch (err) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold text-gray-900 mb-4">Login to TravelMind</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="text-center text-sm mt-4">
          ...
          <button type="button" onClick={onSwitchToSignup} className="text-black underline">
            Sign up
          </button>
        </div>
      </form>
